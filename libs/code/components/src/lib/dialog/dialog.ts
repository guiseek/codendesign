...
export class Dialog implements OnDestroy {
  ...
  openFromComponent<T>(
    ...
    const dialogContainer = this._attachCodeDialogContainer(overlayRef, config);
  ...
  /**
   * Anexa um FloatCodeDialogContainer à sobreposição já criada de uma caixa de diálogo.
   ...
   */
  protected _attachCodeDialogContainer(
    overlay: OverlayRef,
    config: DialogConfig
  ): CodeDialogContainer {
    const container =
    ...
    const containerRef: ComponentRef<CodeDialogContainer> = overlay.attach(
      containerPortal
    );
  ...
  /**
   * Anexa o componente fornecido pelo usuário ao FloatCodeDialogContainer já criado.
   ...
   * @param dialogContainer Referência ao FloatCodeDialogContainer de quebra automática.
  ...
  protected _attachDialogContentForComponent<T>(
    ...
    dialogContainer: CodeDialogContainer,
  ...
  private _applyConfigDefaults(config?: DialogConfig): DialogConfig {
    const dialogConfig = (this._injector.get(
      DIALOG_CONFIG
    ) as unknown) as typeof DialogConfig;
